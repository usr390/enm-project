...
export class EnmEventListToolbarComponent {
  ...
  showToolbar$ = this.store$.select(RouterSelectors.selectUrl).pipe(
    map(url => url && (url.startsWith('/events') || url.startsWith('/checkout/return') || url.startsWith('/plus') || url.startsWith('/artist-directory') || url.startsWith('/account') || url.startsWith('/termsofservice') || url.startsWith('/faq') || url.startsWith('/privacypolicy') || url.startsWith('/iBhLq5wrrxafte4a')))
  );
