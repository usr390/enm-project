...
def create_multipage_pdf(folder_path, grid_size, image_size, output_path):
    ...
    for page in range(total_pages):
        grid = Image.new('RGB', (image_size * grid_size[0], image_size * grid_size[1]), "white")
        ...
        for y in range(grid_size[1]):
            for x in range(grid_size[0]):
                if i < len(page_images):
                    grid.paste(page_images[i], (x * image_size, y * image_size))
                    i += 1
                else:
                    break
...
image_size = 100  # Resize QR codes to 200x200 pixels
output_path = "printables.pdf"  # Update this to where you want to save the PDF

# Call the function with hardcoded parameters
create_multipage_pdf(folder_path, (grid_width, grid_height), image_size, output_path)
print(f"Multi-page PDF created at {output_path}")
