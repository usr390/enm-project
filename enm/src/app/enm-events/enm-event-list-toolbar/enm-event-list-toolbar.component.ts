...
export class EnmEventListToolbarComponent {
  ...
  showToolbar$ = this.store$.select(RouterSelectors.selectUrl).pipe(
    map(url => url && (url.startsWith('/events') || url.startsWith('/plus') || url.startsWith('/iBhlq5wrrxefte1e') || url.startsWith('/account') || url.startsWith('/termsofservice') || url.startsWith('/faq') || url.startsWith('/privacypolicy')))
  );
